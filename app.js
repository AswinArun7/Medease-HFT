...
const mongoURI = process.env.MONGODB_URI || "mongodb+srv://<username:password>@cluster.mongodb.net/dbname?retryWrites=true&w=majority&appName=HFT";
...
mongoose.connection.on('error', (err) => {
  ...
  if (err.message.includes("authentication failed") || err.message.includes("bad auth")) {
    console.error('⚠ AUTHENTICATION ERROR: Password is incorrect!')
  ...
  if (!serverStarted) {
    ...
    app.listen(port, () => {
      if (mongoose.connection.readyState === 1) {
        console.log(`✓ Server listening on http://localhost:${port} (MongoDB connected)`);
      } else {
        console.log(`⚠ Server listening on http://localhost:${port} (MongoDB not connected)`);
        console.log(`⚠ Database operations will fail until MongoDB connection is established`);
      }
    });
  }
}
